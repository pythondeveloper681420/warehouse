...
class DataFilterApp:
   ...
   def _init_data(self):
       ...
       if st.session_state.auto_refresh and current_time - st.session_state.last_refresh >= timedelta(minutes=10):
       ...
       with st.spinner("Loading and merging data..."):
           try:
               ...
               if not xml_df.is_empty() and not po_df.is_empty():
                   ...
                   po_cols = ['Purchasing Document_cleaned', 'Project Code', 'Andritz WBS Element', 'Cost Center']
       ...
       for column in selected_columns:
           ...
           if column_type == pl.Utf8:
               if filter_key not in filter_states:
                   filter_states[filter_key] = {'type': 'Input', 'value': []}
               ...
               if filter_type == "Input":
                   value = st.text_input(
                       ...
                       value=','.join(filter_states[filter_key].get('value', []))
                   )
                   if value:
                       filters[column] = ("input", value.split(','))
                       filter_states[filter_key] = {'type': 'Input', 'value': value.split(',')}
               else:
       ...
       for column, (filter_type, value) in filters.items():
           ...
           if filter_type == "input" and filtered_df[column].dtype == pl.Utf8:
               filtered_df = filtered_df.filter(
                   pl.col(column).apply(lambda x: any(v.lower() in str(x).lower() for v in value))
               )
